...
<div id="mySidebar" class="sidebar">
  <a href="javascript:void(0)" class="closebtn" onclick="closeNav()"> x</a>
   <a href="#m1">Models</a>
	<div id="list">
	<a href="lowtohigh.jsp?model=new"><h5><li>New Models</li></h5></a>
	<a href="lowtohigh.jsp?model=up"><h5><li>Up Coming</li></h5></a>
	</div>
  <a href="#m1">Price</a>
	<div id="list">
...
<%
	 ...
	 String low = "low";
	 String under = "10";
	 String between = "10-15";
	 String above = "15";
	 String model = request.getParameter("model");
	 String high1 = request.getParameter("high");
	 String low1 = request.getParameter("low");
	 String under1 = request.getParameter("under");
	 String between1 = request.getParameter("between");
	 String above1 = request.getParameter("above");
	 System.out.println(above1);
 
 %>   
 
 <%
 if(high.equals(high1)){
...
<div class="card-columns">
  ...
  <div class="card">
    <img class="card-img-top" src="image/<%=rs.getString("filename")%>" alt="Card image cap" height="200px" width="150px">
    <div class="card-body">
      ...
      <a href="pyment.jsp?model=<%=rs.getString("model")%>&price=<%=rs.getString("price")%>&mileage=<%=rs.getString("mileage")%>&image=<%=rs.getString("filename")%>&brand=<%=rs.getString("brands")%>"><button type="button" class="btn btn-outline-danger">Book a Car</button></a>   
      <a href="uappointment.jsp?model=<%=rs.getString("model")%>&price=<%=rs.getString("price")%>&mileage=<%=rs.getString("mileage")%>&image=<%=rs.getString("filename")%>&brand=<%=rs.getString("brands")%>"><button class="btn btn-outline-danger">Book Appointment</button></a>    
 ...
 <%
 if(low.equals(low)){
	 String q = "select * from models order by price asc";
	 Statement st = con.createStatement();
	 ResultSet rs = st.executeQuery(q);
%>
...
<div class="card-columns">
  ...
  <div class="card">
    <img class="card-img-top" src="image/<%=rs.getString("path")%>" alt="Card image cap" height="200px" width="150px">
    <div class="card-body">
      <h5 class="card-title"><%=rs.getString("model")%></h5>
      <p class="card-text">Rs. <%=rs.getString("price")%> </br><%=rs.getString("mileage")%> kmpl</br><%=rs.getString("cc")%> cc</br><%=rs.getString("seating")%> seater</br></p>
	
      <a href="pyment.jsp?model=<%=rs.getString("model")%>&price=<%=rs.getString("price")%>&mileage=<%=rs.getString("mileage")%>&image=<%=rs.getString("path")%>&brand=<%=rs.getString("brands")%>"><button type="button" class="btn btn-outline-danger">Book a Car</button></a>   
      <a href="uappointment.jsp?model=<%=rs.getString("model")%>&price=<%=rs.getString("price")%>&mileage=<%=rs.getString("mileage")%>&image=<%=rs.getString("path")%>&brand=<%=rs.getString("brands")%>"><button class="btn btn-outline-danger">Book Appointment</button></a>    
  ...
  <%
 if(under.equals(under1)){
 
 
 String q = "select * from models where mileage<10";
 Statement st = con.createStatement();
 ResultSet rs = st.executeQuery(q);
...
<div class="card-columns">
  ...
  <%
 if(between.equals(between1)){
 
 
 String q = "select * from models where mileage>10 and mileage<15";
 Statement st = con.createStatement();
 ResultSet rs = st.executeQuery(q);
...
<div class="card-columns">
  ...
  <%
 if(above.equals(above1)){
 
 
 String q = "select * from models where mileage>=15";
 Statement st = con.createStatement();
 ResultSet rs = st.executeQuery(q);
